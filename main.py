...
def predict_image(network, drawer, image, label):
    drawer.saveImage(image, "PNG")
    test = read_image(image)
    print(test)
    result = output[np.argmax(network.predict(np.asarray(test)))]
    print(result)
...
if __name__ == '__main__':
    ...
    newNetwork = NeuralNetwork(W1, b1, W2, b2)

    print(newNetwork.calc_accuracy(dataset))
    test = read_image("test.png")
    print(output[np.argmax(newNetwork.predict(np.asarray(test)))])

    app = QApplication(sys.argv)
    w = QWidget()
    btnSave = QPushButton("Определить фигуру")
    ...
    btnSave.clicked.connect(lambda: predict_image(newNetwork, drawer, "image.png", label))
